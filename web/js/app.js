const App = {
    ...
    async loadArticles() {
        ...
        try {
            ...
            grid.innerHTML = '<div class="loading-state">加载失败，请重试</div>';
    ...
    renderArticles(articles) {
        ...
        if (!articles || articles.length === 0) {
            grid.innerHTML = `<div class="empty-state">
                <h3>暂无内容</h3>
                <p>${this.state.activeCategory ? '该领域下暂无文章，试试查看其他领域' :
                  `数据源尚未采集完成，运行 <code>python crawler.py</code> 开始采集`}</p>
            </div>`;
            return;
        }
    ...
    _renderArticle(a) {
        ...
        return `
        <div class="article-card" onclick="window.open('${this._escapeAttr(a.url)}', '_blank')">
            ...
            <div class="card-footer">
                <a href="${this._escapeAttr(a.url)}" target="_blank" class="card-link" onclick="event.stopPropagation()">
                    阅读原文 →
                </a>
            </div>
        </div>`;
    ...
    updateFilters() {
        ...
        if (this.state.activeCategory) {
            html += `<span class="filter-pill category">
                📁 ${this.state.activeCategory}
                <span class="remove-filter" onclick="App.selectCategory(null)">✕</span>
            </span>`;
        }

        if (this.state.activeTag) {
            html += `<span class="filter-pill tag">
                # ${this.state.activeTag}
                <span class="remove-filter" onclick="App.selectTag(null)">✕</span>
            </span>`;
        }
        ...
        if (this.state.searchTerm) {
            html += `<span class="filter-pill category">
                ...
                <span class="remove-filter" onclick="document.getElementById('searchInput').value='';App.search()">✕</span>
            </span>`;
        }
    ...
    renderPagination() {
        ...
        if (totalPages <= 1) {
            container.innerHTML = totalArticles > 0
                ? `<span class="page-info">共 ${totalArticles} 条</span>`
                : "";
            return;
        }
    ...
    async subscribe() {
        ...
        if (!email || !email.includes("@") || !email.includes(".")) {
            this._showSubscribeMsg("请输入有效的邮箱地址", "error");
            return;
        }
        ...
        try {
            ...
            this._showSubscribeMsg(data.message || "订阅成功", data.status === "success" ? "success" : "error");
            if (data.status === "success") {
                emailInput.value = "";
                setTimeout(() => { this.loadSubscriberCount(); }, 2000);
            }
        } catch (e) {
            this._showSubscribeMsg("网络错误，请稍后重试", "error");
        } finally {
            btn.disabled = false;
            btn.textContent = "订阅";
        }
